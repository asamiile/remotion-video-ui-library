...
- For `CodeStream`, keep the content split across **`config/local/composition-text.example.json`** and **`config/local/composition-text.local.json`** under `codeStreamV1Patterns`, with runtime wiring in **`src/composition/`**.
