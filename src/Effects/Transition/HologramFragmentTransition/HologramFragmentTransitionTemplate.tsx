...
import {hologramFragmentTransitionAnimationDurationFrames, type HologramFragmentTransitionProps} from "./hologram-fragment-transition.schema";
export const HologramFragmentTransitionTemplate: React.FC<HologramFragmentTransitionProps> = ({primaryColor, secondaryColor, fragmentCount, spreadPx, fragmentSize, mode, randomSeed}) => {
  ...
  const animationStartFrame = Math.floor((durationInFrames - hologramFragmentTransitionAnimationDurationFrames) / 2);
  const animationFrame = frame - animationStartFrame;
  if (animationFrame < 0 || animationFrame >= hologramFragmentTransitionAnimationDurationFrames) return null;
  let p = interpolate(animationFrame, [0, hologramFragmentTransitionAnimationDurationFrames - 1], [0, 1]); if (mode === "reassemble") p = 1 - p;
