...
export const TextlessSciFiOverlayTemplate: React.FC<TextlessSciFiOverlaySchemaType> = ({
  ...
  const glitchBlockDisplacement = () => <>{range(Math.round(20 * density)).map((i) => {
    ...
    return <g key={i} transform={`translate(${(r(`gbs${i}`)-.5)*90*intensity} 0)`}><rect x={x} y={y} width={w} height={h} fill={i % 3 === 0 ? accentColor : primaryColor} opacity={.1 * opacity}/><line x1={x} x2={x+w} y1={y+h/2} y2={y+h/2} stroke={secondaryColor} opacity={.5 * opacity}/></g>;
  })}</>;
  ...
  const radialInterfacePulse = () => <g transform={`translate(${width/2} ${height/2})`} opacity={opacity}>
